...
window.onload = function () {
    // grab all of the parameters from the URL
...
//MORE IN CATEGORY CODE
const moreMealsBtn = document.querySelector("#more-meals");
moreMealsBtn.addEventListener('click', () => {
    //console.log(currentCategory);
    const similarMeals = meals.filter((item) => {
        return item.category === currentCategory && item.id != currentId
    }).map(item => {
    ...
    //<div id="similar-meals-container">x</div>
    ...
    //console.log(similarMealsContainer);
...
const toggleTheme=()=>{
  ...
  const btnContainer = document.querySelector(".buttons-container") 
  const mealName = document.querySelector(".current-meal-info") 
  ...
  if(body.style.backgroundColor==="black"){
    ...
    mealName.querySelector("h2").style.color = "white";
  }
